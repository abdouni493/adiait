...
describe("monthlyExpiry — une carte, jamais un jour de plus", () => {
  it("une carte commencé le 16/08 court jusqu'au 15/09 inclus", () => {
    expect(monthlyExpiry("2026-08-16")).toBe("2026-09-15");
  });

  it("fin de carte court : le 31/01 tombe sur le 27/02 (février tronqué)", () => {
    expect(monthlyExpiry("2026-01-31")).toBe("2026-02-27");
  });

  it("plusieurs cartes d'affilée", () => {
    expect(monthlyExpiry("2026-08-16", 3)).toBe("2026-11-15");
  });
});

describe("formule par carte du tarif", () => {
  it("absente tant qu'aucune séance n'est comprise dans la carte", () => {
  ...
  it("prix de la carte par défaut = séances × prix d'une séance", () => {
    expect(monthlyPriceOf(sub({ monthlySeances: 8 }))).toBe(4000);
  });

  it("le prix saisi par le club l'emporte sur le total calculé", () => {
...
describe("expiration d'une inscription à la carte", () => {
  it("les séances restent disponibles pendant la carte", () => {
  ...
  it("la carte écoulé emporte les séances non consommées", () => {
