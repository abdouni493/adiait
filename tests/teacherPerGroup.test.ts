 ...
 *
 * Le tarif de l'entraîneur n'est pas sur sa fiche : il est fixé emploi du temps
 * par emploi du temps, sur l'abonnement (prix de la carte → part du club → le
 * reste pour l'entraîneur ÷ séances de la carte). Chaque présence lui rapporte
 * exactement ce tarif, quel que soit le nombre de chevaliers ou le prix payé.
 *
 * Dans le seed, ses-1 / sub-1 vaut 600 DA la séance et reverse 250 DA à
 * l'entraîneur ; tea-1 l'assure, au pourcentage (60 %). Sans le tarif du
 * groupe il toucherait donc 360 DA — c'est ce chiffre qui trahit une régression.
 */

const TEACHER = "tea-1";

/** Les dettes entraîneur ouvertes sur un couple (emploi, chevalier). */
...
/** Bascule un entraîneur sur la formule « par groupe » : plus aucun taux à lui. */
...
/** Retire tout partage club / entraîneur d'un abonnement : le groupe n'est plus tarifé. */
...
describe("tarif porté par l'emploi du temps", () => {
  it("sub-1 reverse 250 DA par séance à l'entraîneur", () => {
...
describe("entraîneur payé par groupe", () => {
  ...
  it("ne touche RIEN sur un emploi du temps dont l'abonnement n'a pas de part entraîneur", async () => {
    makePerGroup(TEACHER);
    ...
    // Aucune dette entraîneur n'est ouverte : le groupe n'est pas encore tarifé,
    // et sa fiche ne porte aucun taux de repli.
    expect(duesOf("ses-2", "stu-3").length).toBe(before);
  });

  it("là où un entraîneur au pourcentage, lui, aurait été payé", async () => {
    // Même emploi non tarifé, mais tea-1 reste au pourcentage : sa fiche prend
    // le relais et il touche ses 60 % du prix réellement facturé au chevalier.
  ...
  it("garde son tarif quel que soit le prix payé par le chevalier", async () => {
    makePerGroup(TEACHER);
    await useData
      .getState()
      .setPresence({ studentId: "stu-1", sessionId: "ses-1", date: "2026-08-13", status: "present" });

    // Le tarif ne dépend ni du prix de la séance ni du solde du chevalier : il est
