...
insert into public.app_page_actions (page_key, action_id, position, label, hint) values
  ...
  -- relancer les impayes n'a pas a lire le chiffre d'affaires du club.
  ('semesters', 'totals',   7, 'Voir l''argent encaisse', 'Le detail chevalier par chevalier, lui, reste lisible dans la liste d''un emploi du temps.'),
  ('semesters', 'whatsapp', 8, 'Envoyer un message WhatsApp', 'Relance individuelle ou envoi groupe aux endettes.'),
  ...
  ('horses', 'view', 2, 'Voir le detail d''un cheval', null),
  ('horses', 'edit', 3, 'Modifier une fiche de cheval', null),
  ('horses', 'delete', 4, 'Supprimer un cheval', null),
  ('horses', 'sell', 5, 'Vendre un cheval', 'Au comptant ou a credit, avec remise.'),
  ('horses', 'pay', 6, 'Encaisser une vente a credit', null),
  ('horses', 'print', 7, 'Imprimer un bon de vente ou un recu', null),
  ('stable', 'create', 1, 'Ajouter un cheval sans achat', null),
  ('stable', 'view', 2, 'Voir le detail et le suivi d''un cheval', null),
  ('stable', 'edit', 3, 'Modifier une fiche de cheval', null),
  ('stable', 'delete', 4, 'Supprimer un cheval', null),
  ('stable', 'expense', 5, 'Porter une depense sur un cheval', 'Cheval du club : sortie de caisse. Cheval en pension : dette du proprietaire.'),
  ('stable', 'pay', 6, 'Encaisser un proprietaire', null),
  ('stable', 'print', 7, 'Imprimer un releve ou un recu', null),
  ('stable-reports', 'print', 1, 'Imprimer le rapport', null),
  ('other-debts', 'create', 1, 'Creer une dette', null),
  ('other-debts', 'view', 2, 'Voir le detail et l''historique', null),
  ('other-debts', 'edit', 3, 'Modifier une dette', null),
  ('other-debts', 'delete', 4, 'Supprimer une dette', null),
  ('other-debts', 'pay', 5, 'Encaisser un reglement', null),
  ('other-debts', 'print', 6, 'Imprimer un releve ou un recu', null),
  ('cash-secondary', 'deposit', 1, 'Depot en caisse secondaire', null),
...
create table if not exists public.cash_transactions (
  ...
  type             text not null check (type in (
                     ...
                     -- L'écurie : l'achat sort, la vente et ses versements
                     -- entrent, l'entretien d'un cheval DU CLUB sort, le
                     -- règlement d'un pensionnaire entre.
  ...
  -- LA CAISSE QUI A SAISI LE MOUVEMENT. Le club tient deux postes de saisie :
  -- la caisse GÉNÉRALE, qui voit tout, et une caisse SECONDAIRE — celle des
  -- travailleurs — qui ne voit que ce qu'elle a elle-même saisi.
  --
  -- L'ARGENT, LUI, N'EST PAS SÉPARÉ : les deux alimentent la même trésorerie,
  -- et la générale affiche tous les mouvements en indiquant lequel vient d'où.
  --
  -- NULLABLE, et c'est délibéré : `null` vaut « générale », ce qui est le cas de
  -- tout l'historique antérieur à cette distinction.
  caisse           text check (caisse is null or caisse in ('general','secondary')),
  created_by       text,
  created_by_name  text,
  created_by_role  text
);
...
-- =============================================================================
--  5 bis. L'ÉCURIE, LES AUTRES DETTES ET LA PASSERELLE WHATSAPP
-- =============================================================================
--
--  LA RÈGLE QUI GOUVERNE L'ÉCURIE EST LE PROPRIÉTAIRE :
--
...
--      propriétaire (`horse_expenses.owner_debt = true`), et la caisse ne bouge
--      qu'au moment où il règle (`horse_owner_payments`).
--
--  Le rattachement à une FICHE du club n'est pas décoratif : c'est lui, et lui
--  seul, qui fait remonter la dette sur le compte de l'intéressé, dans son
--  espace, à côté de ce qu'il doit pour ses cotisations.
-- =============================================================================

-- ---- Les chevaux ------------------------------------------------------------
-- TOUT EST FACULTATIF SAUF LE NOM. Une écurie ne connaît pas la robe, la taille
-- et le carnet de vaccination de chaque cheval le jour où il arrive ; exiger
-- vingt champs ferait saisir vingt approximations.
...
create table if not exists public.horses (
  ...
  -- L'achat sort de la caisse le jour indiqué ; le modifier AJUSTE ce mouvement
  ...
  -- Une INTENTION : le prix affiché tant que personne n'a acheté. La vente
  -- réelle porte son propre montant, remise comprise.
  ...
  -- `on delete set null` : supprimer une fiche de chevalier ne doit jamais faire
  -- disparaître un cheval. Il devient « propriétaire inconnu », ce qui se
  -- corrige — là où une suppression en cascade ne se répare pas.
...
comment on table public.horses is
  'Les chevaux du club et ceux qu''il héberge. Le propriétaire décide de tout : cheval du club = dépenses sur la caisse ; cheval en pension = dépenses portées à son compte.';
...
create table if not exists public.horse_expenses (
  id               text primary key,
  horse_id         text references public.horses (id) on delete cascade,
  -- `on delete set null` + nom recopié : supprimer une rubrique ne doit pas
  -- rendre illisible un historique vieux de deux ans.
  ...
  -- Ce n'est pas un réglage : la valeur est DÉDUITE du propriétaire du cheval.
...
comment on table public.horse_owner_payments is
  'Ce qu''un propriétaire de cheval en pension a versé. Sa dette vaut : somme des dépenses `owner_debt` moins somme de ces versements.';
...
-- ---- Les autres dettes ------------------------------------------------------
-- Ce que l'on doit au club sans que ce soit une cotisation : un fournisseur
-- avancé, une casse à rembourser, du matériel prêté non rendu. Ces sommes
-- n'appartiennent à aucun emploi du temps et n'ont leur place ni sur une carte,
-- ni sur un frais de chevalier.
...
-- ---- La passerelle WhatsApp : DEUX tables, et elles ne se confondent pas -----
--
--  `whatsapp_messages` — LE JOURNAL : ce qui a été confié à la passerelle, avec
--  le TEXTE réellement envoyé et l'avancement de la remise.
--  `whatsapp_outbox`   — LA FILE : ce qui n'a PAS pu partir, avec son texte.
--
--  LA FILE N'EST PAS UN RAFFINEMENT. La passerelle vit sur un poste de l'écurie,
--  et ce poste sera éteint un jour ou l'autre : sans elle, chaque message émis
--  pendant ce temps est PUREMENT PERDU, et un rappel automatique ne laisse rien
--  derrière lui — personne ne revient l'envoyer à la main.
--
...
--
--  Elles sont écrites PAR LE SERVEUR avec la clé de service : le webhook vient
--  de la passerelle, il n'a aucune session utilisateur et ne peut pas écrire
--  sous RLS.
...
create table if not exists public.whatsapp_outbox (
  id               text primary key,
  recipient_name   text not null default '',
  -- Le MSISDN brut attendu par la passerelle (213555123456), et non le rendu
  -- lisible : c'est cette valeur-là que la reprise renvoie telle quelle.
  recipient_phone  text not null default '',
  body             text not null default '',
  status           text not null default 'pending'
  ...
  for r in
    select * from (values
      ...
      ('cash_transactions',           staff_read,
        $w$public.can_write(array['cash','cash-secondary','dashboard','students','teachers','workers','expenses','independent','attendance','horses','stable','other-debts'])$w$),
      ...
      -- L'ÉCURIE. La lecture des familles est le point délicat : un parent doit
      -- voir SON cheval et SA dette dans son espace — c'est toute la raison du
      -- rattachement à une fiche — et ne voir que les siens.
      ('horses',
        $r$public.is_staff()
      ...
      ('horse_expense_categories',     any_signed,
      ...
      -- LES AUTRES DETTES, mêmes règles de lecture.
      ('other_debts',
        $r$public.is_staff()
...
-- =============================================================================
--  LE TEMPS REEL — les statuts de remise WhatsApp avancent sans recharger
-- =============================================================================
--  L'ajout a une publication echoue si la table y est deja : chaque instruction
--  porte donc son propre bloc, pour qu'un rejeu n'annule pas ce qui precede.
