        ...
        {toast.studentName && (
          <div className="bg-canvas/30 rounded-xl p-2.5 space-y-1 text-xs border border-line mt-2">
            ...
            {/* A presence costs ONE séance, never money. `cost` carries the
                séance price (it sizes the teacher's share) and is not shown. */}
            {toast.cost !== undefined && toast.cost > 0 && (
              <div className="flex justify-between">
                <span className="text-muted">Consommé:</span>
                <strong className="text-danger font-semibold">1 séance</strong>
              </div>
            )}
            ...
            {/* `newBalance` carries the SÉANCES left on the inscription — the
                money balance it used to hold no longer exists. */}
            {toast.newBalance !== undefined && (
              <div className="flex justify-between">
                <span className="text-muted">Séances restantes:</span>
