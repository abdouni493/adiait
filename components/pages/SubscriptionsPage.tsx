...
export function SubscriptionsPage() {
  ...
  // monthly formula: N séances sold as ONE month, at its own price
  const [monthlyEnabled, setMonthlyEnabled] = useState(false);
  const [monthlySeances, setMonthlySeances] = useState<number>(0);
  const [monthlyPrice, setMonthlyPrice] = useState<number>(0);
  /** the month price follows the séances until the user types his own */
  const [monthlyPriceDirty, setMonthlyPriceDirty] = useState(false);
  /** how much of the month price the school keeps; the rest goes to the teacher */
    ...
    const divergent = siblings.filter((se) => {
      ...
      return (
        ...
        (other.pricePerSession !== sub.pricePerSession ||
          ...
          (other.monthlySeances ?? 0) !== (sub.monthlySeances ?? 0) ||
          monthlyPriceOf(other) !== monthlyPriceOf(sub))
      );
    });
  ...
  // Revenue of a subscription = every séance PURCHASE made on any group of the
  // course (Σ net total). Attendance no longer moves money — it only burns
  // séances — so the sale is the only thing worth counting here.
  const calculateSubscriptionGains = (sub: Subscription) => {
    ...
    const courseSubIds = new Set(
      subscriptions.filter((x) => siblingSessionIds.has(x.sessionId)).map((x) => x.id),
    );
    const courseEnrollmentIds = new Set(
      enrollments.filter((e) => courseSubIds.has(e.subscriptionId)).map((e) => e.id),
    );
    return payments
      .filter((p) => p.type === "subscription_payment" && p.enrollmentId && courseEnrollmentIds.has(p.enrollmentId))
  ...
  /** One tariff written on EVERY group of the course.
   *  The price of ONE séance is what everything else is built on — it is what
   *  the séance-purchase flow multiplies and what sizes the teacher's share —
   *  so it is required for cours AND formations alike. A formation simply adds
   *  its level price and its duration on top. */
  const applyPrice = async (sessionId: string) => {
    const isFormation = isFormationSession(sessionId);
    if (pricePerSession <= 0) {
      alert("Veuillez saisir le prix d'une séance.");
      return false;
    }
    ...
    const res = await setSubscriptionPrice(sessionId, pricePerSession, {
      ...
      monthlySeances: monthlyEnabled ? monthlySeances : 0,
  ...
  const handleHarmonize = async (sub: Subscription) => {
    setBusy(true);
    await setSubscriptionPrice(sub.sessionId, sub.pricePerSession, {
      levelPrice: sub.levelPrice,
      periodMonths: sub.periodMonths,
      monthlySeances: sub.monthlySeances,
      monthlyPrice: sub.monthlyPrice,
      schoolMonthShare: sub.schoolMonthShare,
      teacherPerSeance: sub.teacherPerSeance,
    });
    setBusy(false);
  };
  ...
  const resetForm = () => {
    ...
    setMonthlySeances(0);
    setMonthlyPrice(0);
    setMonthlyPriceDirty(false);
    setSchoolMonthShare(0);
    setSearchQuery("");
    setSelectedSub(null);
  };

  /** Reloads the whole tariff (séance + monthly formula) into the form. */
  const loadTariff = (sub: Subscription) => {
    setPricePerSession(sub.pricePerSession);
    setLevelPrice(sub.levelPrice ?? 0);
    setPeriodMonths(sub.periodMonths ?? 0);
    setMonthlyEnabled(hasMonthlyPlan(sub));
    setMonthlySeances(sub.monthlySeances ?? 0);
    setMonthlyPrice(monthlyPriceOf(sub));
    setMonthlyPriceDirty(hasMonthlyPlan(sub));
    // Default the school share to the whole month price on legacy tariffs that
    // never had a split, so the teacher share reads 0 until the user sets one.
    setSchoolMonthShare(hasMonthlyPlan(sub) ? schoolMonthShareOf(sub) : 0);
  };
  ...
  /** Séances of a month bought one by one — what the pack is compared to. */
  const monthComputedTotal = Math.max(0, Math.round(monthlySeances * pricePerSession));
  const monthSaving = monthComputedTotal - monthlyPrice;

  /** Ticking N séances per month proposes the matching total straight away; the
   *  user stays free to sell the pack cheaper (or dearer) by typing his own. */
  const changeMonthlySeances = (value: number) => {
    const count = Math.max(0, Math.round(value || 0));
    setMonthlySeances(count);
    if (!monthlyPriceDirty) setMonthlyPrice(Math.max(0, Math.round(count * pricePerSession)));
  };

  const changePricePerSession = (value: number) => {
    const price = Number(value) || 0;
    setPricePerSession(price);
    if (!monthlyPriceDirty) setMonthlyPrice(Math.max(0, Math.round(monthlySeances * price)));
  };
   ...
   */
  const renderMonthlyBlock = () => (
    <div
      className={`rounded-xl border p-3 space-y-3 ${
        monthlyEnabled ? "border-primary/30 bg-primary-50/40" : "border-line bg-canvas/30"
      }`}
    >
      <label className="flex cursor-pointer items-start gap-2 text-xs">
        <input
          ...
          onChange={(e) => {
            ...
            if (e.target.checked && monthlySeances <= 0) changeMonthlySeances(8);
          }}
      ...
      {monthlyEnabled && (
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <Input
                type="number"
                min={1}
                value={monthlySeances || ""}
                onChange={(e) => changeMonthlySeances(Number(e.target.value))}
                placeholder="Ex: 8"
              />
            </div>
            <div>
              ...
              {monthlyPriceDirty && (
                <button
                  onClick={() => {
                    setMonthlyPriceDirty(false);
                    setMonthlyPrice(monthComputedTotal);
                  }}
          ...
          {/* School / teacher split of the month price. The school share is the
              only number the user types — the teacher's share and his pay per
    ...
    <div>
      ...
      <Modal open={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Créer un tarif d'abonnement" wide>
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-muted mb-1">Prix d&apos;une séance (DA)</label>
            <Input
              type="number"
              value={pricePerSession || ""}
              onChange={(e) => changePricePerSession(Number(e.target.value))}
              placeholder="Ex: 500"
            />
