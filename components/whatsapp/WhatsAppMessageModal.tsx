...
export function WhatsAppMessageModal({
    ...
    return getTemplate(nextTemplateId).build(
      {
        ...
        remainingSeances: student?.remainingSeances ?? 0,
        debt: student?.debt ?? 0,
  ...
  const handleSend = async () => {
    ...
    // Mode démo : aucune passerelle Meta n'est branchée. Le message est
    // composé et validé normalement, mais rien ne part sur le réseau.
    void buildOutgoing();
    void stripId;
    addToast({
      type: "info",
      title: "WhatsApp désactivé en mode démo",
      message: `${chosen.length} destinataire(s) sélectionné(s) — aucun message n'est réellement envoyé.`,
    });
    setSending(false);
    onClose();
  };
